...
def draw_chart(row):
    ...
    fig.add_hline(y=max_vol_price, line_width=2, line_dash="dash", line_color="orange", 
                  annotation_text=f" 🚧 {max_vol_price}円 (需給の壁) ", annotation_position="top left", row=1, col=1)
    fig.add_hline(y=recent_20_low, line_width=1.5, line_dash="dot", line_color="cyan", 
                  annotation_text=f" 🔵 {recent_20_low}円 (直近底値) ", annotation_position="bottom left", row=1, col=1)
...
with tab1:
    ...
    if search_btn and input_code:
        codes = [c.strip() for c in input_code.replace(',', ' ').split()]
        for code in codes:
            ...
            if data:
                with st.container():
                    ...
                    with c1:
                        # ランクと記号を横並びに
                        st.markdown(f"#### 総合判定: <span style='color:orange;'>{data['ランク']}</span> &nbsp;&nbsp; {data['icons_str']}", unsafe_allow_html=True)
                    ...
                    with c2:
                        st.markdown(f"🛡️ **底値乖離率: {data['乖離率']}%**")
                        st.info("💡 過去20営業日(約1ヶ月)の最安値を基準に算出しています。20%を超えると高値掴みのリスクが高まります。")
                        draw_chart(data)
...
with tab2:
    ...
    if st.button("🚀 スキャン開始"):
        ...
        for i, code in enumerate(jpx_codes[:100]): # デモ用に件数制限
        ...
        if results:
            for r in results:
                with st.expander(f"【{r['ランク']}】 {r['icons_str']} {r['コード']} {r['銘柄名']}"):
                    st.write(f"現在値: {r['現在値']}円 | 介入度: {r['intervention_score']}%")
                    draw_chart(r)
